/*!
 * koa-better-method-override -- koa@2.x.x
 *
 ...
 * the method in `ctx.request.body[getter]` and a function will be
 * called with `ctx.request` and expects the method to be returned.
 * If the string starts with `X-` then it will look in
 * `ctx.request.headers[getter]` instead.
 *
 * The original method is available via `ctx.request.originalMethod`.
 *
...
module.exports = function methodOverride (getter, options = {}) {
  ...
  return function methodOverride (ctx, next) {
    ctx.request.originalMethod = ctx.request.originalMethod || ctx.request.method

    // validate request is an allowed method
    if (methods && methods.indexOf(ctx.request.originalMethod) === -1) {
      return next()
    }

    const val = get(ctx.request, ctx.response)
    const method = Array.isArray(val) ? val[0] : val

    // replace
    if (method !== undefined && supports(method)) {
      ctx.request.method = method.toUpperCase()
      debug(`override ${ctx.request.originalMethod} as ${ctx.request.method}`)
    }
